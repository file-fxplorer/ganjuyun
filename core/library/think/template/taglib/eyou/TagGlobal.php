                ...
                default:
                    if (preg_match('/^web_attrname_(\d+)$/i', $name)) {
                        ...
                        if (null === $config_attribute) {
                            $config_attribute = Db::name('config_attribute')->field('attr_id,attr_name')->where(['lang'=>self::$home_lang])->getAllWithIndex('attr_id');
                        }
                        $attr_id = str_replace('web_attrname_', '', $name);
                        $attr_id = intval($attr_id);
                        $value = !empty($config_attribute[$attr_id]) ? $config_attribute[$attr_id]['attr_name'] : '';
                    } else {
                        /*支持子目录*/
