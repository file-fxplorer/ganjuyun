...
class TagList extends Base
{
    ...
    public function getList($param = array(), $pagesize = 10, $orderby = '', $addfields = '', $ordermode = '', $thumb = '', $arcrank = '')
    {
        ...
        $cacheKey = 'taglib-'.md5(__CLASS__.__FUNCTION__.json_encode([$param,$pagesize,$orderby,$addfields,$ordermode,$thumb,$arcrank,$this->tid,self::$main_lang,self::$home_lang,self::$city_switch_on,self::$site_info,$web_stypeid_open,$site_showall]));
        ...
        foreach (['keywords','keyword','typeid','notypeid','channel','flag','noflag'] as $key) {
            if (isset($param[$key]) && $param[$key] !== '') {
                ...
                elseif ($key == 'typeid') {
                    $param[$key] = str_replace('，', ',', $param[$key]);
                    if (empty($web_stypeid_open)) { // 关闭副栏目
                        $condition['a.typeid'] = array('in', $param[$key]);
                    } else { // 开启副栏目
                        $stypeid_where = "";
                        $typeid_arr = explode(',', $param[$key]);
                        foreach ($typeid_arr as $_k => $_v) {
                            $stypeid_where .= " OR CONCAT(',', a.stypeid, ',') LIKE '%,{$_v},%' ";
                        }
                        $condition[] = Db::raw(" (a.typeid IN ({$param[$key]}) {$stypeid_where}) ");
                    }
                }
                elseif ($key == 'channel') {
                    $param[$key] = str_replace('，', ',', $param[$key]);
