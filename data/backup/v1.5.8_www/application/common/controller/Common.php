...
class Common extends Controller {
    ...
    public function _initialize() 
    {
        ...
        if (!empty($global['web_users_switch'])) {
            ...
            /*会员登录有效期逻辑*/
            if (session('?users_id')) {
                $users_login_expiretime = !empty($this->usersConfig['users_login_expiretime']) ? $this->usersConfig['users_login_expiretime'] : 3600;
                $users_login_expire = session('users_login_expire'); //最后登录时间
            ...
            /*end*/
            if ( !session('?users_id') || ($times - intval($users_login_expire)) > $users_login_expiretime ) {
                session('users_id', null);
                session('users', null);
                cookie('users_id', null);
            } else if ( session('?users_id') && ($times - intval($users_login_expire)) < $users_login_expiretime ) {
