...
class ApiLogic extends Model
{
    ...
    public $taglib = ['apiType','apiChannel','apiList','apiArclist','apiArcview','apiPrenext','apiGuestbookform',
        ...
        'apiMediaOrderList','apiMediaOrderDetails','apiPlayList','apiUpgradeLevelOrderList','apiPickUpPointsList'];
    ...
    public function taglibData($users = [])
    {
        ...
        foreach ($params as $key => $val) {
            ...
            }else if ($key == 'apiPickUpPointsList') {  // 提货点列表
                $pagesize = !empty($parse['pagesize']) ? $parse['pagesize'] : 20;
                $page = !empty($parse['page']) ? $parse['page'] : 1;
                $tagList = new \think\template\taglib\api\TagPickUpPointsList;
                $result[$key][$ekey] = $tagList->apiPickUpPointsList($page, $pagesize, $parse);
            }
        }
