...
//                        $list[$key]['product_spec'] .= $spec_value['spec_name'] . '：' . $spec_value['spec_value'] . '；';
    ...
    public function ShopOrderPay($post = [], $level_discount = 100)
    {
        ...
        $OrderData = model('ShopPublicHandle')->orderSubmitPublicHandle($OrderData, $post, $this->users_id, $list);
        // dump($OrderData);exit;
    ...
    {
        $notify_url = !empty($notify_url) ? $notify_url : url('api/v1/Api/wxpay_notify', [], true, true, 1, 2);
        // 当前时间戳
        $time = time();
...
//                $filter['is_comment'] = 0;
            ...
            if ('wechat' == $param['pay_type']) {
                ...
                if (!empty($ReturnID)) {
                    ...
                    $money_insert = [
                        'users_id' => $this->users_id,
                        'money' => $price,
                        'users_money' => $users['users_id']-$price,
                        'cause' => '视频购买，使用余额支付，订单号：'.$OrderNumber,
                        'cause_type' => '3',
                        'status' => 3,
                        'order_number' => $OrderNumber,
                        'add_time' => getTime(),
                    ...
                    Db::name('users_money')->insert($money_insert);
    ...
    public function WechatAppletsPayDealWithMedia($PostData = [], $notify = false)
    {
        ...
        if (!empty($moneyid) && !empty($order_code)) {
            $where = [
                'users_id'   => $UsersID,
                'order_code' => $order_code,
            ];
