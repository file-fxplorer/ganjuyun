    ...
    public function SendMobileCode()
    {
        ...
        if (IS_AJAX_POST) {
            ...
            // 订单付款和订单发货类型发送短信处理
                ...
                switch ($data['type']) {
                    case '1':
                        ...
                        $content =  !empty( $data['order_code']) ?  $data['order_code'] : "";//$preg_res ? '待发货' : '您有新的待发货订单';
                        break;
                    case '2':
            ...
            //发送表单提醒
                ...
                $sms_type = $sms_config['sms_type'] ? : 1;
                $tpl_content = Db::name('sms_template')->where(["send_scene"=> $post['scene'],"sms_type"=> $sms_type])->value('tpl_content');
                if (!$tpl_content) return false;
...
/*
    ...
    public function ajax_get_content($aid=0)
    {
        ...
        $artData = Db::name('archives')
            ->alias('a')
            ->field('a.restric_type,a.arc_level_id,a.users_price,a.no_vip_pay,a.add_time,b.content,b.content_ey_m')
        ...
        if (empty($artData['restric_type'])) { // 免费
            ...
            if (empty($users_id)) {
                ...
                $UsersData = GetUsersLatestData();
                //会员限制
                if (1 < $artData['arc_level_id'] && $UsersData['level'] < $artData['arc_level_id']) {
...
//                                return $uplaod_data['errmsg'];
    ...
    public function get_download($aid=0)
    {
        ...
        if (empty($artData['restric_type'])) { // 免费
            ...
            $usersInfo = Db::name('users')->field('a.level,b.level_name')
                ->alias('a')
                ->join('users_level b','a.level=b.level_id','left')
                ->where('a.users_id',$users_id)
                ->find();
            if ($usersInfo['level'] >= $artData['arc_level_id']){
                $canDownload = 1;
                $download_tips = "您已是{$usersInfo['level_name']}，可直接下载";
            }else{
